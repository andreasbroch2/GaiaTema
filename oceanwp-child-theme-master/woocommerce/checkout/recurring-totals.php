...
?>
			...
			<tr class="recurring-totals">
				<th colspan="2"><?php esc_html_e( 'Faste Betalinger', 'woocommerce-subscriptions' ); ?></th>
			</tr>
		...
		<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
			...
			<tr class="order-total recurring-total">
				<?php if ( $display_th ) : $display_th = false; ?>
					<th rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>"><?php esc_html_e( 'Fast Beløb', 'woocommerce-subscriptions' ); ?></th>
					<td data-title="<?php esc_attr_e( 'Fast Beløb', 'woocommerce-subscriptions' ); ?>"><?php wcs_cart_totals_order_total_html( $recurring_cart ); ?></td>
				<?php else : ?>
